	...
	setInterval(async () => {
		...
		if (d.getMinutes() === 0 && d.getSeconds() === 0) {
			...
			const { body } = await snekfetch.get(
				'https://www.reddit.com/r/dankmemes/top.json?sort=top&t=24hours'
			);
			const allowed = sendChannel.nsfw
				? body.data.children
		...
		if (d.getMinutes() === 0 && d.getSeconds() === 0) {
			...
			// get the json version of the memes from r/dankmemes/top
			const { body } = await snekfetch.get(
				'https://www.reddit.com/r/dankmark/top.json?sort=top&t=24hours'
			);
			const allowed = sendChannel.nsfw
				? body.data.children
				: body.data.children.filter((post) => !post.data.over_18);
...
bot.on('guildMemberAdd', async (member) => {
	...
	let amEmbed = new Discord.RichEmbed()
		.setAuthor(
			`${member.user.username}#${member.user.discriminator} (ID: ${member.id})`,
...
bot.on('channelCreate', async (channel) => {
	...
	let logChannel = channel.guild.channels.find(
		(c) => c.name === 'server-log'
	);

	let ccEmbed = new Discord.RichEmbed()
		.setTitle(
			`Channel "#${channel.name}" has been created by ${entry.executor.username}#${entry.executor.discriminator}!`
		)
		.setColor(botSettings.green)
...
bot.on('channelDelete', async (channel) => {
	...
	let cdEmbed = new Discord.RichEmbed()
		.setTitle(
			`Channel "#${channel.name}" has been deleted by ${entry.executor.username}#${entry.executor.discriminator}!`
		)
		.setColor(botSettings.red)
		.addField('Channel:', channel, true)
...
bot.on('guildMemberUpdate', async (oldMember, newMember) => {
  ...
  */
	...
	let updateEmbed = new Discord.RichEmbed().setTitle(
		`Changes to ${oldMember.user.username}#${oldMember.user.discriminator}`
	);
	...
	if (oldMember.nickname !== newMember.nickname) {
		...
		updateEmbed.setDescription(
			`Nickname was changed for ${oldMember.user}`
		);
...
bot.on('messageDelete', async (message) => {
	...
	if (message.attachments.find((a) => a) != null) {
		deleteEmbed.addField(
			'Picture:',
			message.attachments.find((a) => a).proxyURL
		);
	}
	...
	try {
		if (
			message.embeds !== [] &&
			message.embeds.find((t) => t).image != null
		) {
			console.log(message.embeds.find((t) => t).image);
			deleteEmbed.addField(
				'Picture from embed:',
				message.embeds.find((t) => t).image.proxyURL
			);
		}
	} catch (e) {
		// console.log(e)
...
bot.on('channelPinsUpdate', async (channel, time) => {
	...
	let pinEmbed = new Discord.RichEmbed()
		.addField(
			`A pin has been ${addRemove}`,
			`A pin has been ${addRemove} in ${channel} - ${time}`
		)
...
bot.on('message', async (message) => {
	...
	if (message.content.startsWith(defaultPrefix + 'reset_prefix')) {
		if (!message.member.hasPermission('MANAGE_GUILD'))
			return errors.noPerms(message, 'MANAGE_SERVER');
	...
	if (message.content.startsWith(prefix + 'play')) {
		...
		var link = '';
		if (url2 === 'mission_impossible')
			link = 'https://www.youtube.com/watch?v=XAYhNHhxN0A';
		else if (url2 === 'mc')
			link = 'https://www.youtube.com/watch?v=CBbvpgr4Qb4';
		else if (url2 === 'cocaine')
			link = 'https://www.youtube.com/watch?v=1bSQfyYVNW8';
		else if (url2 === 'die')
			link = 'https://www.youtube.com/watch?v=kJVLSPxj43k';
		else if (url2 === 'james_bond')
			link = 'https://www.youtube.com/watch?v=ye8KvYKn9-0';
		else if (url2 === 'kazoo')
			link = 'https://www.youtube.com/watch?v=7eVO_3z-Lhc';
		else if (url2 === 'minecraft_theme')
			link = 'https://www.youtube.com/watch?v=cjQQ9JYGgTM';
		else if (url2 === '#1')
			link = 'https://www.youtube.com/watch?v=PfYnvDL0Qcw';
		else if (url2 === 'max')
			link = 'https://www.youtube.com/watch?v=wLuE_yvUO1U';
		else if (url2 === 'rollin')
			link = 'https://www.youtube.com/watch?v=CtwJvgPJ9xw';
		else if (url2 === 'hentai')
			link = 'https://www.youtube.com/watch?v=1ER67r8OCW8';
		else if (url2 === 'ocean_man')
			link = 'https://www.youtube.com/watch?v=tkzY_VwNIek';
		else if (url2 === 'kappa')
			link = 'https://www.youtube.com/watch?v=XpTZlXn3pxY';
		else if (url2 === 'anime')
			link =
				'https://www.youtube.com/playlist?list=PLyIUbA7gq1Mp5aCinh7ClZ3KY-QtP1zrK';
		else if (url2 === 'chill' || url2 === 'code')
			link = 'https://www.youtube.com/watch?v=vAKtNV8KcWg';
		else if (url2 === 'chill2' || url2 === 'code2')
			link = 'https://www.youtube.com/watch?v=hHW1oY26kxQ';
		else if (url2 === 'ddak')
			link =
				'https://www.youtube.com/playlist?list=PLNbz9Y6Tg2_z0e7H5RGQ0VaCqtejy-y2c';
		else if (url2 === 'maxdak' || url2 === 'sot')
			link =
				'https://www.youtube.com/playlist?list=PL0cWlOyqP6_PKyS76fDsafkj_ho4KLmEA';
		else if (url2 === 'og' || url2 === 'OG')
			link =
				'https://www.youtube.com/playlist?list=PLNbz9Y6Tg2_weCm_9ZE6OmJ0Mbag5e7AF';
		else if (url2 === 'lose_it' || url2 === 'lube_it')
			link = 'https://www.youtube.com/watch?v=TGaXcdvZ3qA';
		else return;

		const voiceChannel = message.member.voiceChannel;
		if (!voiceChannel)
			return message.channel.send(
				"I'm sorry but you need to be in a voiceChannel to play music"
			);
			...
			try {
				...
				try {
					message.channel.send(
						`<:YouTube:521795667392200716> Searching :mag_right: **${searchString}**`
					);
...
Please provide a value to select one of the 🔎 results ranging from **1-5**.`);
		...
		if (!message.member.voiceChannel)
			return message.channel.send("You're not in a voiceChannel!");
		if (!serverQueue)
			return message.channel.send(
...
function play(guild, song) {
	...
	serverQueue.textChannel.send(
		`:church: :notes: Playing: **${song.title}** - now`
	);
}
// End of YouTube bot
